...
export default function AdminOrderDetailPage() {
  ...
  async function srAction(action: () => Promise<any>, successMsg: string) {
    ...
    try {
      await action()
      setSrMsg({ type: 'ok', text: successMsg })
      await load()
    } catch (e: any) {
      setSrMsg({ type: 'err', text: e?.message || 'Action failed' })
  ...
  async function handlePush() {
    await srAction(
      ...
      'Order pushed to ShipRocket and AWB assigned!'
    )
  ...
  async function handleTrack() {
    ...
    try {
      const data = await adminApi.trackShipment(id)
      setTracking(data)
    } catch (e: any) {
      setSrMsg({ type: 'err', text: `Tracking failed: ${e?.message}` })
  ...
  async function handleRefund() {
    ...
    try {
      ...
      setRefundMsg({ type: 'ok', text: 'Refund initiated successfully.' })
      await load()
    } catch (e: any) {
      setRefundMsg({ type: 'err', text: e?.message || 'Refund failed' })
  ...
  // Parse ShipRocket tracking activities
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <span className={`text-[10px] uppercase tracking-widest px-2 py-0.5 rounded ${STATUS_COLOR[order.status] || 'bg-gray-100 text-gray-500'}`}>
          {order.status}
        </span>
        {order.shiprocket_status && (
          <span className="text-[10px] uppercase tracking-widest px-2 py-0.5 rounded bg-orange-100 text-orange-700">
            SR: {order.shiprocket_status}
          </span>
        )}
      </div>

      {/* Top grid: Customer + Payment + Address + Fulfillment */}
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white border border-[#e8e4e0] p-5">
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span className="uppercase text-[10px] tracking-widest">{order.payment_method || '—'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#6b6b6b]">Status</span>
              <span className={`font-medium ${PAY_COLOR[order.payment_status] || ''}`}>{order.payment_status || '—'}</span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="bg-white border border-[#e8e4e0] p-5">
          ...
          ) : <p className="text-sm text-[#6b6b6b]">No address recorded</p>}
        </div>
        ...
        <div className="bg-white border border-[#e8e4e0] p-5">
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-[#6b6b6b]">Fulfillment</span>
              <span className="uppercase text-[10px] tracking-widest">{order.fulfillment_status || 'unfulfilled'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#6b6b6b]">Placed</span>
        ...
        <table className="w-full text-sm font-sans">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-widest text-[#6b6b6b] border-b border-[#e8e4e0]">
              ...
              <th className="px-5 pb-2 text-right">Qty</th>
              <th className="px-5 pb-2 text-right">Price</th>
              <th className="px-5 pb-2 text-right">Subtotal</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item: any, i: number) => (
              <tr key={i} className="border-b border-[#f0ece8] last:border-0">
                <td className="px-5 py-3 text-[#0a0a0a]">{item.snapshot_name}</td>
          ...
          <tfoot>
            <tr className="border-t border-[#e8e4e0]">
              ...
              <td className="px-5 py-3 text-right font-semibold text-[#0a0a0a]">{formatPrice(order.total)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div className="bg-white border border-[#e8e4e0] mb-8">
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-[#e8e4e0]">
          <div className="flex items-center gap-2">
            ...
            {inSR && (
              <span className="text-[10px] font-mono text-[#6b6b6b]">#{order.shiprocket_order_id}</span>
            )}
          </div>
          {inSR && !['cancelled','refunded'].includes(order.status) && (
            <button
              onClick={() => srAction(() => adminApi.cancelShipment(id), 'Shipment cancelled in ShipRocket')}
              disabled={srLoading}
        ...
        <div className="p-5">
          ...
          {inSR && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
              <div>
                <p className="text-[10px] uppercase tracking-widest text-[#6b6b6b] mb-1">AWB</p>
                <p className="font-mono text-sm text-[#0a0a0a]">{order.awb_code || '—'}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-widest text-[#6b6b6b] mb-1">Courier</p>
                <p className="text-sm text-[#0a0a0a]">{order.courier_name || '—'}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-widest text-[#6b6b6b] mb-1">SR Status</p>
                <p className="text-sm text-[#0a0a0a]">{order.shiprocket_status || '—'}</p>
              </div>
              <div>
                ...
                {order.label_url ? (
                  <a href={order.label_url} target="_blank" rel="noopener noreferrer"
                    className="text-sm underline text-[#c8a4a5]">Download ↗</a>
                ) : (
                  <button
                    disabled={srLoading}
                    onClick={() => srAction(() => adminApi.generateLabel(id), 'Label generated')}
                    className="text-sm underline text-[#0a0a0a] disabled:opacity-40"
                  >
                    Generate
                  </button>
                )}
          ...
          <div className="flex flex-wrap gap-3">
            {!inSR && order.status !== 'cancelled' && order.status !== 'refunded' && (
              <button
            ...
            {inSR && !hasAwb && (
              <button
                disabled={srLoading}
                onClick={() => srAction(() => adminApi.assignAwb(id), 'AWB assigned')}
            ...
            {inSR && hasAwb && !order.pickup_scheduled_at && (
              <button
                disabled={srLoading}
                onClick={() => srAction(() => adminApi.schedulePickup(id), 'Pickup scheduled!')}
                className="px-4 py-2 text-sm border border-[#0a0a0a] hover:bg-[#0a0a0a] hover:text-white transition-colors disabled:opacity-40"
              >
                Schedule Pickup
              </button>
          ...
          {showPushForm && (
            <div className="mt-5 bg-[#faf8f5] border border-[#e8e4e0] p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                {[
                  { label: 'Weight (kg)', value: pkgWeight, set: setPkgWeight },
                  { label: 'Length (cm)', value: pkgLength, set: setPkgLength },
                  { label: 'Breadth (cm)', value: pkgBreadth, set: setPkgBreadth },
                  { label: 'Height (cm)', value: pkgHeight, set: setPkgHeight },
                ].map(f => (
                  <div key={f.label}>
                    <label className="block text-[10px] uppercase tracking-widest text-[#6b6b6b] mb-1">{f.label}</label>
                    <input type="number" step="0.1" min="0.1" value={f.value}
              ...
              <div className="mb-3">
                <label className="block text-[10px] uppercase tracking-widest text-[#6b6b6b] mb-1">Courier ID (optional — leave blank for auto)</label>
                <input type="number" value={courierId} onChange={e => setCourierId(e.target.value)}
                  placeholder="e.g. 13 for Delhivery"
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowPushForm(false)}
                  className="px-4 py-2 text-sm border border-[#e8e4e0] hover:bg-gray-50">
                  Cancel
                </button>
              </div>
            </div>
              ...
              <div className="relative pl-4 border-l-2 border-[#e8e4e0] space-y-4">
                {activities.map((act: any, i: number) => (
                  <div key={i} className="relative">
                    ...
                    <p className="text-sm font-medium text-[#0a0a0a]">{act['sr-status'] || act.status}</p>
                    <p className="text-xs text-[#6b6b6b]">{act.location} — {act.date}</p>
                    {act.activity && <p className="text-xs text-[#6b6b6b] italic">{act.activity}</p>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {tracking && !activities.length && (
            <div className="mt-4 bg-[#faf8f5] border border-[#e8e4e0] p-3 text-xs font-mono text-[#6b6b6b] overflow-x-auto">
              <pre>{JSON.stringify(tracking, null, 2).slice(0, 800)}</pre>
            </div>
          )}
        </div>
      </div>
      ...
      {canRefund && (
        <div className="bg-white border border-[#e8e4e0] p-5 mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-[11px] uppercase tracking-widest text-[#6b6b6b] mb-1">
                Amount (₹) — blank = full refund
              </label>
              <input type="number" step="0.01" min="1" value={refundAmt}
          ...
          <button onClick={handleRefund} disabled={refunding}
            className="px-5 py-2 text-sm bg-[#c62828] text-white hover:bg-[#b71c1c] transition-colors disabled:opacity-50">
            {refunding ? 'Processing…' : 'Issue Refund'}
          </button>
          <p className="text-[11px] text-[#6b6b6b] mt-2">
            {order.payment_method === 'cod' ? 'COD — manual refund record will be created.' : 'Razorpay refund initiated immediately.'}
          </p>
        </div>
      )}
      {order.payment_status === 'refunded' && (
        <div className="bg-[#f3e5f5] border border-[#ce93d8] px-5 py-4 text-sm text-purple-800">
