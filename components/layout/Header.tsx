...
export default function Header() {
  ...
  return (
    <>
      <header className={`sticky top-0 z-30 bg-white transition-all duration-300 ${scrolled ? 'shadow-[0_2px_20px_rgba(0,0,0,0.07)]' : 'border-b border-[#e8e4e0]'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <Link href="/" className="group flex flex-col items-center gap-0">
              <span className="font-serif text-[22px] tracking-[0.35em] text-[#0a0a0a] uppercase leading-none">KALOKEA</span>
              {/* Underline accent — grows on hover */}
              <span className="h-px bg-[#c8a4a5] w-0 group-hover:w-full transition-all duration-500 ease-out" />
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-5 lg:gap-7">
              {NAV.map((n) => (
                <Link
                  key={n.href}
                  href={n.href}
                  className={`nav-link text-[10px] font-sans tracking-widest uppercase transition-colors pb-0.5 ${
                    n.rose
                      ? 'text-[#c8a4a5] hover:text-[#a07e80]'
                      : 'text-[#0a0a0a] hover:text-[#c8a4a5]'
                  }`}
                >
                  {n.label}
                </Link>
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-50 flex flex-col animate-fade-in">
          ...
          <div className="relative bg-white border-b border-[#e8e4e0] px-4 sm:px-6 py-5 shadow-float">
            <form onSubmit={handleSearch} className="max-w-2xl mx-auto flex items-center gap-3">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#9b9b9b" strokeWidth="1.5" strokeLinecap="round" className="shrink-0">
                <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
              </svg>
              <input
                ref={searchRef}
              ...
              />
              {searchQuery && (
                <button type="button" onClick={() => setSearchQuery('')} className="min-w-[44px] min-h-[44px] flex items-center justify-center text-[#9b9b9b] hover:text-[#0a0a0a] text-xl leading-none">×</button>
              )}
              <button type="submit" disabled={!searchQuery.trim()} className="btn-shimmer px-5 py-2 text-[11px] uppercase tracking-widest bg-[#0a0a0a] text-white hover:bg-[#1a1a1a] disabled:opacity-40 transition-colors shrink-0 relative overflow-hidden">
                Search
              </button>
            </form>
            <div className="max-w-2xl mx-auto mt-3 flex flex-wrap gap-2 pl-7">
              {['Dresses', 'Tops', 'Sale', 'New Arrivals'].map(term => (
                <button
                  key={term}
                  onClick={() => { setSearchOpen(false); router.push(`/shop?category=${term.toLowerCase().replace(/ /g, '-')}`) }}
